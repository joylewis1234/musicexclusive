...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Step 1: Link auth_user_id on the application record ──
    ...
    if (linkError) {
      ...
      console.log("[finalize-artist-setup] ✅ Linked auth_user_id to application for:", email);
    }

    // ── Step 2: Update application status to active ──
    ...
    // ── Step 3: Ensure artist role exists ──
    ...
    // ── Step 4: Check if artist profile exists; if not, create one ──
